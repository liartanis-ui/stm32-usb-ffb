  ...
  switch (req->bmRequest & USB_REQ_TYPE_MASK)
  {
    case USB_REQ_TYPE_CLASS:
      switch (req->bRequest)
      {
        ...
        {
          uint8_t report_id = LOBYTE(req->wValue);  // ȡ Report ID
          printf("GET_REPORT type=%02X id=%02X len=%d\r\n",
                  HIBYTE(req->wValue), report_id, req->wLength);

          if (((USBD_CUSTOM_HID_ItfTypeDef *)pdev->pUserData[pdev->classId])->GetReport != NULL)
          {
            ReportLength = req->wLength;
            pbuf = ((USBD_CUSTOM_HID_ItfTypeDef *)pdev->pUserData[pdev->classId])->GetReport(report_id, &ReportLength);
          }
